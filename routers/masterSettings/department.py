...
from models.DepartmentModel import Department, DepartmentCollection
...
async def create(department: Department = Body(...)):
    try:
        new_department = await department_collection.insert_one(department.model_dump())
        new_department_data = await department_collection.find_one({"_id": new_department.inserted_id})
        new_department_data = convert_objectid_to_str(new_department_data)
        new_department_data["_id"] = str(new_department_data["_id"])
        return {"success": True, "message": "Department created successfully", "data": new_department_data}
...
async def list_pagination_invoice(page: int = 1, search: str = ''):
    try:
        ...
        results = await department_collection.find(options).skip(skip).limit(limit).to_list(limit)
